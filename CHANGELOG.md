...
- **PDF classification pipeline bypass**: PDFs (and other document formats) were all routing to `0_Inbox (80% extension_router)` instead of being classified. Three compounding bugs fixed:
  ...
  - **MLX native LLM disabled** (`.env`): Enabled `PARA_FILES_MLX_LLM_ENABLED=true` to activate the local Qwen2.5 LLM fallback via `mlx-lm`; changed default from `False` to `True` so LLM fallback works out-of-the-box
  - **Nested config models ignored .env file** (`config.py`): `MLXConfig`, `LLMConfig`, `LoggingConfig`, and `ExtensionRoutingConfig` did not have `env_file=".env"` — since `load_config()` pre-instantiates them before the root `Config`, they never read `.env` values. Added `env_file=".env"` to all nested `BaseSettings` models
  - **MLX-LM generate API breakage** (`mlx_llm_classifier.py`): Replaced deprecated `temp=0.0` kwarg with `sampler=make_sampler(temp=0.0)` — `mlx-lm >=0.22` removed the `temp` parameter from `generate_step()`, causing `TypeError` at inference time
  - **LLM hallucinated invalid paths** (`mlx_llm_classifier.py`, `pipeline.py`): The Qwen2.5 LLM was producing paths like `C:/Users/...`, `/home/shivkumar/...`, and `archives/factures/2023/Denver` because the static system prompt lacked real PARA categories. Fixed by building a dynamic prompt with all 49 valid category patterns from `documents.json` and adding `_sanitize_category()` output validation that rejects absolute paths, Windows paths, and categories without a valid PARA prefix
